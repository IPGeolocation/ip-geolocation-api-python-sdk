# ipgeolocation.AbuseContactApi

All URIs are relative to *https://api.ipgeolocation.io/v2*

| Method                                                                  | HTTP request   | Description                                                     |
|-------------------------------------------------------------------------|----------------|-----------------------------------------------------------------|
| [**get_abuse_contact_info**](AbuseContactApi.md#get_abuse_contact_info) | **GET** /abuse | Get abuse reporting contact information for a given IP address. |


# **get_abuse_contact_info**
> AbuseResponse get_abuse_contact_info(ip=ip, excludes=excludes, fields=fields)
...
with ipgeolocation.ApiClient(configuration) as api_client:
    ...
    api_instance = ipgeolocation.AbuseContactApi(api_client)
    ip = '8.8.8.8' # str | query parameter 'ip'. (optional)
    excludes = 'abuse.handle,abuse.emails' # str | You can exclude specific fields from the API response (except the ip field) by listing them in the excludes parameter as a comma-separated list. For example, you want to remove emails and handle from api response, you can put the keys in excludes parameter like this. (optional)
    fields = 'abuse.role,abuse.emails' # str | You can customize the API response by using the fields parameter to include only the specific data you need. For example, to retrieve only the role and emails, specify these keys in the fields parameter as shown below. (optional)

    try:
        api_response = api_instance.get_abuse_contact_info(ip=ip, excludes=excludes, fields=fields)
        print("The response of AbuseContactApi->get_abuse_contact_info:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AbuseContactApi->get_abuse_contact_info: %s\n" % e)
```
 ...
 **excludes** | **str**| You can exclude specific fields from the API response (except the ip field) by listing them in the excludes parameter as a comma-separated list. For example, you want to remove emails and handle from api response, you can put the keys in excludes parameter like this. | [optional] 
 **fields** | **str**| You can customize the API response by using the fields parameter to include only the specific data you need. For example, to retrieve only the role and emails, specify these keys in the fields parameter as shown below. | [optional] 
